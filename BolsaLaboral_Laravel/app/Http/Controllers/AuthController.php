...
class AuthController extends Controller
{
    ...
    {
        $this->middleware('auth:api', ['except' => ['login', 'registerCompany', 'register', 'registerUser']]);
    }
    /**
    ...
    public function register(Request $request)
    {
        ...
        $roles = Role::whereIn('roleName', ['admin', 'company', 'user'])->get();
    ...
    public function registerCompany(Request $request)
    {
        ...
        $roles = Role::where('roleName', ['company'])->first();
        $user->roles()->attach($roles);


        return response()->json([
            'status' => true,
            'message' => 'Company user successfully registered',
            'user' => $user
        ], 201);
    }
    /**
